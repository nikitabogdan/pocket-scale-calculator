...
fun PocketScaleCalculator.convertDeviceScalePlaceholder() = (if (poModel == PocketOperators.PO_33) {
    ""
} else {
    prepareScaleNameText(PO35Scales.MINOR)
}
...
fun PocketScaleCalculator.convertScaleNamePlaceholder() = "${rootKey.dropOctave()} ${scale.scaleName}"
    .addCharsToFullLine(SCALE_NAME_PLACEHOLDER.length)
